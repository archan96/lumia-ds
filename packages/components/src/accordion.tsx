...
export type AccordionProps = AccordionPrimitive.AccordionProps & {
  type?: AccordionPrimitive.AccordionProps['type'];
};
    ...
    <AccordionPrimitive.Header className="flex">
      <AccordionPrimitive.Trigger
        ...
        className={cn(
          ...
          'data-[state=open]:text-foreground data-[state=closed]:text-muted',
          className,
        )}
...
>(function AccordionContent({ className, children, ...props }, ref) {
  return (
    <AccordionPrimitive.Content
      ...
      className={cn(
        'grid overflow-hidden px-4 text-sm text-muted transition-[grid-template-rows,opacity] duration-200 ease-out',
