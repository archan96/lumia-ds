...
describe('Tailwind preset', () => {
    ...
    it('maps color tokens to CSS variables with fallbacks', () => {
        ...
        expect(colors.secondary).toBe(`var(--color-secondary, ${cssVars['--color-secondary']})`);
        expect(colors.background).toBe(`var(--color-bg, ${cssVars['--color-bg']})`);
        expect(colors.foreground).toBe(`var(--color-fg, ${cssVars['--color-fg']})`);
        expect(colors.border).toBe(`var(--color-border, ${cssVars['--color-border']})`);
    ...
    it('maps radii and typography to CSS variables', () => {
        const preset = createLumiaTailwindConfig();
        const { borderRadius, fontFamily } = (preset.theme as {
            extend: {
        ...
        expect(borderRadius.DEFAULT).toBe(`var(--radius-md, ${defaultTheme.radii.md})`);
        expect(borderRadius.sm).toBe(`var(--radius-sm, ${defaultTheme.radii.sm})`);
        expect(borderRadius.pill).toBe(`var(--radius-pill, ${defaultTheme.radii.pill})`);
